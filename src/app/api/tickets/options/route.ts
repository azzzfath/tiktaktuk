...
export async function GET() {
  ...
  try {
    ...
    const [orders, customers, tickets, categories, events, seats, relationships] = await Promise.all([
      ...
      supabase.from("ticket").select("ticket_id, tcategory_id, torder_id"),
      supabase.from("ticket_category").select("category_id, category_name, price, quota, tevent_id"),
    ...
    const firstTicketByOrder = new Map((tickets.data ?? []).map((ticket) => [ticket.torder_id, ticket]));
    ...
    return NextResponse.json({
      orders: (orders.data ?? []).map((order) => {
        ...
        const category = firstTicket ? categoryById.get(firstTicket.tcategory_id) : null;
      ...
      categories: (categories.data ?? []).map((category) => {
        const event = eventById.get(category.tevent_id);
        ...
        return {
          ...
          sold_count: (tickets.data ?? []).filter((ticket) => ticket.tcategory_id === category.category_id).length,
          event_id: category.tevent_id,
